...
const DataPenyedia = () => {
  ...
  const handleSearch = (event) => {
    ...
    const filtered = data.filter((item) => {
      return (
        (item?.penyedia && item.penyedia.toLowerCase().includes(value)) ||
  ...
  const deletePenyedia = async (id) => {
    await axios({
      method: "delete",
      url: `${import.meta.env.VITE_APP_API_URL}/api/penyedia/${id}`,
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${user?.token}`,
      },
  ...
  const handleConfirmDeletePenyedia = async (id) => {
    return Swal.fire({
      title: "Are you sure?",
      text: "You will Delete This Penyedia!",
      icon: "warning",
      ...
      if (result.value) {
        ...
        Swal.fire({
          icon: "success",
          title: "Deleted!",
          text: "Your Penyedia has been deleted.",
        });
      }
    });
  };
  ...
  const columns = useMemo(
        ...
        cell: (row) => (
          <div className="flex items-center space-x-2">
            <button title="Edit" className="text-[#16B3AC] hover:text-cyan-500">
              <Link
                to={`/master-data-penyedia/edit/${encodeURIComponent(
                  encryptId(row.id)
                )}`}
              >
                <FaEdit size={16} />
            ...
            {user.role == "1" ? (
              <button
                ...
                onClick={() => handleConfirmDeletePenyedia(row.id)}
              >
                <FaTrash size={16} />
              </button>
            ) : (
    ...
    [handleConfirmDeletePenyedia, user.role]
  );

  return (
    <div>
      <Breadcrumb pageName="Data Penyedia" title="Data Penyedia" />
        ...
        <div className="flex justify-between mb-4 items-center">
          ...
          <div className="div flex gap-2 flex-row">
            ...
            {user.role == "1" ? (
              ...
              >
                <Link
                  to="/master-data-penyedia/add"
                  className="flex items-center gap-2 px-4 py-2"
                >
                  <FaPlus size={16} />
                  <span className="hidden sm:block">Tambah Data Penyedia</span>
                </Link>
              </button>
            ) : (
              ""
