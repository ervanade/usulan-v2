...
const DataPeriodeAlkes = () => {
  ...
  const handleSearch = (event) => {
    ...
    const filtered = data.filter((item) => {
      return (
        ...
        (item?.tanggal_from &&
          item.tanggal_from.toLowerCase().includes(value)) ||
        (item?.tanggal_to && item.tanggal_to.toLowerCase().includes(value))
      );
    });
  ...
  const deleteBarang = async (id) => {
    await axios({
      method: "delete",
      url: `${import.meta.env.VITE_APP_API_URL}/api/periodealikes/${id}`,
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${user?.token}`,
      },
        ...
        cell: (row) => (
          <div className="flex items-center space-x-2">
            <button title="Edit" className="text-[#16B3AC] hover:text-cyan-500">
              <Link
                to={`/master-data-periode-alkes/edit/${encodeURIComponent(
                  encryptId(row.id),
                )}`}
              >
                <FaEdit size={16} />
        ...
        <div className="flex justify-between mb-4 items-center">
          ...
          <div className="div flex gap-2 flex-row">
            ...
            {user.role == "1" ? (
              ...
              >
                <Link
                  to="/master-data-periode-alkes/add"
...
export default DataPeriodeAlkes;
