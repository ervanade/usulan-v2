...
const LaporanDetailBarang = () => {
  const user = useSelector((a) => a.auth.user);
  const navigate = useNavigate();
  const { idKabupaten } = useParams();
  ...
  const fetchDataLaporan = async () => {
    ...
    try {
      const response = await axios({
        ...
        data: {
          id_kabupaten:
            encodeURIComponent(decryptId(idKabupaten) || "0").toString() || "0",
  ...
  useEffect(() => {
    fetchKecamatan(encodeURIComponent(decryptId(idKabupaten) || "0"));
    fetchUserData();
    fetchDataLaporan();
    fetchBarang();
  }, []);
...
export default LaporanDetailBarang;
